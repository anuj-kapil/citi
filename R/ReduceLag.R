...
dt_out <- lapply(c(20,30), function(y){
  dt_inner <- lapply(listIndustries, function(x){
    ...
    dt_new[(rowNum-4):rowNum,C20_Adjusted := (fcast$mean * .8)+(C20Pred * .2) ]
    dt_new[(rowNum-4):rowNum,C20_F := fcast$mean]
    dt_new[(rowNum-4):rowNum,C20_F_80_U := fcast$upper[,1]]
    dt_new[(rowNum-4):rowNum,C20_F_80_L := fcast$lower[,1]]
    dt_new[(rowNum-4):rowNum,C20_F_95_U := fcast$upper[,2]]
    dt_new[(rowNum-4):rowNum,C20_F_95_L := fcast$lower[,2]]
    dt_new[,C20_Adjusted:= as.numeric(C20_Adjusted)]
    dt_new[,C20_F:= as.numeric(C20_F)]
    dt_new[,C20_F_80_U:= as.numeric(C20_F_80_U)]
    dt_new[,C20_F_80_L:= as.numeric(C20_F_80_L)]
    dt_new[,C20_F_95_U:= as.numeric(C20_F_95_U)]
    dt_new[,C20_F_95_L:= as.numeric(C20_F_95_L)]
    ...
    dt_new[,C30_ActualGrowth:= round(((C30-shift(C30, type = "lag", n = 1))/shift(C30, type = "lag", n = 1))*100,2)]
